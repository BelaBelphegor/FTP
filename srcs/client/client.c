...
/*   Updated: 2018/10/08 20:03:05 by tiboitel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (client->running)
	{
		while ((bytes_received = recv(client->socket, buffer,  4096- 1, 0)) > 0)
		{
			...
			if (ft_strncmp(client->last_command, "GET", 3) == 0)
			{
				...
				dprintf(1, "Buffer: %s\n", buffer);
				filename = strrchr(client->last_command, ' ') + 1;
			...
			send(client->socket, buffer, ft_strlen(buffer) + 1, 0);
			ft_memcpy(client->last_command, buffer, ft_strlen(buffer));
			ft_bzero(buffer, 4096 - 1);
			bytes_received = 0;
		}
		dprintf(1, "It's ending ?");
	}
	return (-1);
}
